...
extension Dictionary where Key == String, Value == String {
    ...
    var lowerCasedKeys: [String: String] {
        Dictionary(uniqueKeysWithValues: map { ($0.key.lowercased(), $0.value) })
    }
    
    func contains(key: String, value: String? = nil) -> Bool {

        let dictionary = lowerCasedKeys

        if !dictionary.keys.contains(key.lowercased()) {
            return false
        }

        if let value {
            return dictionary[key.lowercased()] == value
        }
